...
export default function CategoryList() {
  ...
  const handleAdd = async (e) => {
    ...
    if (!name) {
      ...
      if (result != null) {
        ...
        showAlert(`Category couldnot added`)
        setName('')
      }
      setName('')
    }
  ...
  async function addCategory(name) {
    try {
      const url = process.env.NEXT_PUBLIC_BACKEND_ENDPOINT + 'admin/addcategory'
      const categoryData = {
        name: name
      }
      const result = await axios.post(url, categoryData, {
  ...
  const getCategorys = async (e) => {
    try {
      ...
      const url = process.env.NEXT_PUBLIC_BACKEND_ENDPOINT + 'admin/getcategory/' + searchingName;
  ...
  const handleDelete = async (id) => {
    try {
      const url = process.env.NEXT_PUBLIC_BACKEND_ENDPOINT + 'admin/deletecategory/' + id
    ...
    <>
      ...
      <AdminLayout>
        <div>
          ...
          <div>
            <h6 className="text-md font-semibold text-center py-4">Add Category</h6>
