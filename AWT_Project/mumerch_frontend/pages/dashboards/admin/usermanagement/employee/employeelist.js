...
export default function AdminList() {
  ...
  async function addUser(name, email, phone, designation) {
    try {
      const url = process.env.NEXT_PUBLIC_BACKEND_ENDPOINT + 'admin/addemployee'
      const userData = {
        name: name,
        email: email,
        phoneNumber: phone,
  ...
  const getUsers = async (e) => {
    try {
      ...
      const url = process.env.NEXT_PUBLIC_BACKEND_ENDPOINT + 'admin/getemployeebyname/' + searchingName;
  ...
  const handleUpdate = (id) => {
    router.push(`./updateemployee?id=${id}`)
  }
  const handleDelete = async (id) => {
    try {
      const url = process.env.NEXT_PUBLIC_BACKEND_ENDPOINT + 'admin/deleteemployee/' + id
  ...
  return (
    <>
      ...
      <AdminLayout>
        <div>
          <h6 className="text-xl font-semibold dark:text-white">Employee Corner</h6>
          <hr className="h-px bg-gray-200 border-1 dark:bg-gray-700" />
          <div>
            <h6 className="text-md font-semibold text-center py-4">Add New Employee</h6>
          ...
          <div>
            ...
            {Array.isArray(users) ? (
              ...
              <div>No Employee created yet</div>
            )
            }
          </div>
        </div>
